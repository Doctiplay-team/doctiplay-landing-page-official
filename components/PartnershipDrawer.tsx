...
import { X, Send, User, Mail, Building2, Briefcase, MessageSquare, CheckCircle2 } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';
import { sendPartnershipEmail } from '../utils/email';
...
const PartnershipDrawer: React.FC<PartnershipDrawerProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const institution = (form.elements[2] as HTMLInputElement).value;
        const role = (form.elements[3] as HTMLInputElement).value;
                    ...
                    {isSuccess ? (
                        <div className="h-full flex flex-col items-center justify-center text-center animate-fade-in">
                            ...
                            <h4 className="text-xl font-display font-bold text-white mb-2 uppercase tracking-tight">
                                {t.partnership_drawer.success_title}
                            </h4>
                            <p className="text-slate-400 text-sm leading-relaxed max-w-[250px]">
                                {t.partnership_drawer.success_message}
                            </p>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest pl-1">{t.partnership_drawer.labels.name}</label>
                                <div className="relative group">
                                    ...
                                    <input
                                        required
                                        type="text"
                                        placeholder={t.partnership_drawer.placeholders.name}
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest pl-1">{t.partnership_drawer.labels.email}</label>
                                <div className="relative group">
                                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-primary transition-colors" />
                                    <input
                                        required
                                        type="email"
                                        placeholder={t.partnership_drawer.placeholders.email}
                                        className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-3.5 pl-11 pr-4 text-white text-sm focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-slate-700"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest pl-1">{t.partnership_drawer.labels.institution}</label>
                                <div className="relative group">
                                    ...
                                    <input
                                        required
                                        type="text"
                                        placeholder={t.partnership_drawer.placeholders.institution}
                                        className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-3.5 pl-11 pr-4 text-white text-sm focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-slate-700"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest pl-1">{t.partnership_drawer.labels.role}</label>
                                <div className="relative group">
                                    <Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-primary transition-colors" />
                                    <input
                                        required
                                        type="text"
                                        placeholder={t.partnership_drawer.placeholders.role}
                                        className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-3.5 pl-11 pr-4 text-white text-sm focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-slate-700"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest pl-1">{t.partnership_drawer.labels.message}</label>
                                <div className="relative group">
                                    <MessageSquare className="absolute left-4 top-5 w-4 h-4 text-slate-500 group-focus-within:text-primary transition-colors" />
                                    <textarea
                                        required
                                        rows={4}
                                        placeholder={t.partnership_drawer.placeholders.message}
                            ...
                            <div className="pt-4">
                                ...
                                >
                                    {isSubmitting ? (
                                        ...
                                        <>
                                            <span>{t.partnership_drawer.btn_submit}</span>
